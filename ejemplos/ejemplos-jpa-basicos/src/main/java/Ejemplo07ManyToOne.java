import es.jllopezalvarez.accesodatos.ejemplos.ejemplosjpabasicos.entities.City;
import es.jllopezalvarez.accesodatos.ejemplos.ejemplosjpabasicos.entities.Country;
...
public class Ejemplo07ManyToOne {
    ...
    private static void showCityAndCountry(int cityId, EntityManager entityManager) {
        ...
        System.out.printf("Ciudad: %d / %s - Actualizado: %s\n", city.getCityId(), city.getCity(), city.getLastUpdate());
        ...
        scanner.nextLine();
        // Mostramos país
        System.out.printf("\tPaís: %d / %s - Actualizado: %s\n", city.getCountry().getCountryId(), city.getCountry().getCountry(), city.getCountry().getLastUpdate());
